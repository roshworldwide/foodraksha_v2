...
export function StaffDesk({
  ...
  /**
   * Every list control lives in the URL, so returning from a client file lands
   * back on the same search, chip, page and sort the staff member left.
   */
  function openClient(applicationId: string) {
          ...
          <tbody>
            {rows.map((row) => (
              <tr
                ...
                onClick={() => openClient(row.applicationId)}
                tabIndex={0}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    openClient(row.applicationId);
                  }
                }}
                aria-label={`Open ${row.customerName}`}
                className="h-[56px] cursor-pointer transition-colors hover:bg-white-titanium-lt"
              >
